...
foreach ($bons as $bon) {
  ...
  $output .= ' <td class="">
           ...
           <a href="bon_commande.php?id=' . $bon['id_bon_commande'] . '" target="_blank"><button class="btn btn-warning  btn-sm"><i class=" fa fa-print "></i></button></a> ';

           if($bon['signature_daf'] == 0 && $bon['signature_daf']==0 && $_SESSION['role'] != 'comptable'){
            $output .= ' <button class="btn btn-danger  btn-sm" onclick="confirmer2(' . $bon['id_bon_commande'] . ','.$bon['id_engagement'].')"><i class="fa fa-trash"></i></button> '; 
  
           }


    if ($_SESSION['role'] == 'daf' && $bon['signature_daf'] == 0) {
      $output .= '   
                <button class="btn btn-primary  btn-sm" id="' . $bon['id_bon_commande'] . '" name="' . $bon['id_bon_commande'] . '" onclick="confirmer(' . $bon['id_bon_commande'] . ')"><i class="fa fa-check"></i></button>';
    }
    if ($_SESSION['role'] == 'dg' && $bon['signature_dg'] == 0) {
      $output .= '   
                <button class="btn btn-primary  btn-sm" id="' . $bon['id_bon_commande'] . '" name="' . $bon['id_bon_commande'] . '" onclick="confirmer(' . $bon['id_bon_commande'] . ')"><i class="fa fa-check"></i></button>';
    }
    if ($_SESSION['role'] == 'admin' && ($bon['signature_daf'] == 0 || $bon['signature_dg'] == 0)) {
      $output .= '   
                <button class="btn btn-primary  btn-sm" id="' . $bon['id_bon_commande'] . '" name="' . $bon['id_bon_commande'] . '" onclick="confirmer(' . $bon['id_bon_commande'] . ')"><i class="fa fa-check"></i></button>';
    }
    if (($bon['signature_daf'] == 1) && ($bon['signature_dg'] == 1)) {
      $output .= ' 
            <a href="maj_bon_commande.php?id='. $bon['id_bon_commande'] . '"><button class="btn btn-success  btn-sm"><i class=" fa fa-edit "></i></button></a> 
           ';
           if($bon['signature_daf'] == 1 && $bon['signature_daf']==1){
      if ($bon['transfert'] == 0) {
        $output .= '   
            <button class="btn btn-primary btn-sm" title="Transférer au fournisseur" id="' . $bon['id_bon_commande'] . '" name="' . $bon['id_bon_commande'] . '" onclick="confirmer1(' . $bon['id_bon_commande'] . ')"><i class="fa fa-paper-plane"></i></button>';
      } else {
        $output .= ' 
            <a href="../livraison/creer_livraison.php?id=' . $bon['id_bon_commande'] . '"><button class="btn btn-success  btn-sm" title="Enregistrer la livraison"><i class="fa fa-truck"></i></button></a> 
           ';
      }
      if($_SESSION['role'] == 'admin'){
        $output .= ' <button class="btn btn-danger  btn-sm" onclick="confirmer(' . $bon['id_bon_commande'] . ','.$bon['id_engagement'].')"><i class="fa fa-trash"></i></button>'; 
      }
    }
...
<body class="hold-transition sidebar-mini layout-navbar-fixed layout-fixed">
  ...
  <script>
    ...
    if (msg == 1) {
      $(function() {
        toastr.success('Bon de commande signé .')
      });
    }
    if (msg == 2) {
      $(function() {
