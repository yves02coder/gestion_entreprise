...
foreach ($produits as $produit) {
  $output .= '  <tr>
            ...
            if($_SESSION['role']!='commercial'){
              $output .= ' <a href="maj_produit.php?id=' . $produit['id_produit'] . '"><button class="btn btn-success  btn-sm"><i class=" fa fa-edit "></i></button></a> 
              <button class="btn btn-danger btn-sm" id="'.$produit['id_produit'].'" name="'.$produit['id_produit'].'" onclick="confirmer(' . $produit['id_produit'] . ')"><i class="fa fa-trash"></i></button>';
            }
...
<body class="hold-transition sidebar-mini layout-navbar-fixed layout-fixed">
            ...
            <div class="col-md-12">
              <div class="card">
                <div class="card-header">
                  <h3 class="card-title">Liste des produits validés</h3>
                </div>
                <!-- /.card-header -->
                <div class="card-body">
                  <?php echo $output; ?>
  ...
  <script>
    ...
    $("#example1").DataTable({
     ...
     if(msg==2){$(function() {
      toastr.error('Produit supprimée.')
    });
     }
