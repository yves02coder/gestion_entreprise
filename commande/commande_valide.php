...
foreach ($commandes as $commande) {
    $output .= '  <tr>
            ...
            <a href="../facture/creer_facture.php?id=' . $commande['id_commande'] . '"><button class="btn btn-warning btn-sm"><i class=" far fa-credit-card " title="Facture"></i></button></a> ';
    if ($_SESSION['role'] != "commercial") {
        $output .= '   <a href="#?id=' . $commande['edit_commande'] . '"><button class="btn btn-success btn-sm"><i class=" fa fa-edit "></i></button></a>  
                <button class="btn btn-danger  btn-sm" onclick="confirmer2(' . $commande['id_commande'] . ',' . $commande['num_proforma'] . ')"><i class="fa fa-trash"></i></button> ';
    }
...
<body class="hold-transition sidebar-mini layout-navbar-fixed layout-fixed">
    ...
    <script>
        ...
        if (msg == 1) {
            $(function() {
                toastr.success('Commande validée avec succès.')
            });
        }
        if (msg == 2) {
            $(function() {
