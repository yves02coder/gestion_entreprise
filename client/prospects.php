...
foreach ($clients as $client) {
  $output .= '  <tr>
            ...
            if($_SESSION['role']!='commercial'){
              $output .= '<button class="btn btn-danger  btn-sm" id="'.$client['id_client'].'" name="'.$client['id_client'].'" onclick="confirmer(' . $client['id_client'] . ')"><i class="fa fa-trash"></i></button>';
...
<body class="hold-transition sidebar-mini layout-navbar-fixed layout-fixed">
  ...
  <script>
    ...
    $("#example1").DataTable({
     ...
     if(msg==1){$(function() {
      toastr.success('Prospect enregistré avec succès.')
    });
     }
     if(msg==2){$(function() {
     ...
     function confirmer(id){
         ...
         if(rep)
            {
             window.location='supprimer_prospect.php?id='+id;        
             }
           else  {
              document.reload();
  }
